...
uniform samplerCubeArray u_textureDiffuse;
...
vec3 brdfLambert(vec2 randomPoint, mat3 basis)
{
	...
	return u_colorMaterial * texture(u_textureSpecular, vec4(L, 0.0)).rgb;
}
...
vec3 brdfCookTorrance(vec2 randomPoint, mat3 basis, vec3 N, vec3 V, float k)
{
	...
	if (NdotL > 0.0 && NdotV > 0.0)
	{
		...
		return texture(u_textureSpecular, vec4(L, 0.0)).rgb * colorFactor;
	}
